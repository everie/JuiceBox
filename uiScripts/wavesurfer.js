    ...
    drawBars: function (peaks, channelIndex, start, end) {
        ...
        [ this.waveCc, this.progressCc ].forEach(function (cc) {
            ...
            for (var i = (start / scale); i < (end / scale); i += step) {
                ...
                var h = Math.round(peak / absmax);
                console.log(h);
                //cc.fillRect(i + $, halfH - h + offsetY, bar + $, h * 2);
                cc.fillRect(i + $, h, bar + $, h * 4);
            }
        }, this);
    },
...
WaveSurfer.util.extend(WaveSurfer.Drawer.MultiCanvas, {
    ...
    drawBars: function (peaks, channelIndex, start, end) {
        ...
        for (var i = (start / scale); i < (end / scale); i += step) {
            ...
            */
            var peak = peaks[Math.round(i * scale * peakIndexScale)] || 0;
            var h = Math.ceil(Math.abs(peak) / absmax * height * 0.5);
            //console.log(h);
            this.fillRect(i + this.halfPixel, height, bar + this.halfPixel, -h * 2);
        }
    },
